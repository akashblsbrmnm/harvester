 ...
 */
int mlo_parseAssociatedDeviceDiagnostics(void *jsonVal, wifi_associated_dev_t **associated_dev, bool **mld_enable_list, char ***band_list, uint32_t *mloAssocDevCount, char **vapIndex)
{
    ...
    if (json == NULL || associated_dev == NULL || mloAssocDevCount == NULL || vapIndex == NULL || mld_enable_list == NULL || band_list == NULL)
    {
    ...
    *vapIndex = NULL;
    ...
    mld_en = (bool *)calloc(totalLinks, sizeof(bool));
    if (mld_en == NULL)
    {
        CcspHarvesterTrace(("RDK_LOG_ERROR, Harvester %s: Memory allocation failed for mld_en %d devices\n", __FUNCTION__, totalLinks));
        free(dev);
        return 1;
    }

    bands = (char **)calloc(totalLinks, sizeof(char*));
    if (bands == NULL)
    {
        CcspHarvesterTrace(("RDK_LOG_ERROR, Harvester %s: Memory allocation failed for bands %d devices\n", __FUNCTION__, totalLinks));
        free(dev);
        free(mld_en);
        return 1;
    }

    for (uint32_t b = 0; b < totalLinks; b++) {
        bands[b] = (char *)calloc(8, sizeof(char));
        if (bands[b] == NULL) {
             CcspHarvesterTrace(("RDK_LOG_ERROR, Harvester %s: Memory allocation failed for band %d\n", __FUNCTION__, b));
             // Cleanup previously allocated
             for(uint32_t c = 0; c < b; c++) free(bands[c]);
             free(bands);
             free(dev);
             free(mld_en);
             return 1;
    ...
    for(i = 0; i < outerArrSize; i++)
    {
        ...
        if (vapItem != NULL && vapItem->valuestring != NULL)
        {
             ...
             // Set the first found VapIndex as the output for compatibility
        ...
        for(j = 0; j < numClients; j++)
        {
            ...
            for(k = 0; k < numLinks; k++)
            {
                ...
                if (jsonItem != NULL) {
                   if(cJSON_IsBool(jsonItem)) mld_en[current_idx] = cJSON_IsTrue(jsonItem);
                   else if(cJSON_IsString(jsonItem)) mld_en[current_idx] = (atoi(jsonItem->valuestring) == 1);
                   else if(cJSON_IsNumber(jsonItem)) mld_en[current_idx] = (jsonItem->valueint == 1);
                }
                ...
                if (jsonItem != NULL && jsonItem->valuestring != NULL)
                {
                    rc = strcpy_s(bands[current_idx], 8, jsonItem->valuestring);
                    ERR_CHK(rc);
                }
