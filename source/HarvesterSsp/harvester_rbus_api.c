...
int rbus_getMloAssociatedDeviceDiagnosticResult(int index, wifi_associated_dev_t **mlo_dev, bool **mld_enable_list, char ***band_list, uint32_t *mloDevCount, char **vapIndex)
{
    ...
    if (mld_enable_list) *mld_enable_list = NULL;
    if (band_list) *band_list = NULL;
    ...
    int parseRet = mlo_parseAssociatedDeviceDiagnostics(jsonVal, mlo_dev, mld_enable_list, band_list, mloDevCount, vapIndex);
